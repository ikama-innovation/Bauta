...
public class MainView extends AppLayout implements JobEventListener, HasDynamicTitle {
	...
	jobGrid.getChildren().forEach(component -> {
	    ...
	    boolean show = (showJobsFrom.getTime() <= startTime && showJobsTo.getTime() >= startTime) || showJobsFrom.getTime() <= endTime && showJobsTo.getTime() >= endTime || runningJobs.contains(jobName);
	...
	cbShowTimeList.addValueChangeListener(event -> {
		...
		showJobsFrom = Date.from(LocalDateTime.now().with(LocalTime.MIDNIGHT).atZone(ZoneId.systemDefault()).toInstant());
		showJobsTo = new Date();
	...
	for (JobInstanceInfo job : jobDetails) {
	    ...
	    sb.append(job.getName()).append(SEP4).append(job.getExecutionStatus()).append(SEP).append(DurationFormatUtils.formatDuration(totalDuration, "HH:mm:ss")).append(SEP)
		    .append(job.getStartTime() != null ? DateFormatUtils.format(job.getStartTime(), "yyyy-MM-dd HH:mm:ss") : "").append(SEP).append(job.getEndTime() != null ? DateFormatUtils.format(job.getEndTime(), "yyyy-MM-dd HH:mm:ss") : "")
		...
		if (step.getSplitId() != null && splitDurations.containsKey(step.getSplitId())) {
		    ...
		    sb.append(SEP).append(step.getSplitId()).append(SEP).append(SEP3).append(duration != null ? DurationFormatUtils.formatDuration(duration, "HH:mm:ss") : "").append(EOL);
		}
		if (step.getFlowId() != null && flowDurations.containsKey(step.getFlowId())) {
		    ...
		    sb.append(SEP).append(step.getSplitId()).append(SEP).append(alias).append(SEP).append(SEP2).append(duration != null ? DurationFormatUtils.formatDuration(duration.longValue(), "HH:mm:ss") : "").append(EOL);
		}
		sb.append(SEP).append(StringUtils.trimToEmpty(step.getSplitId())).append(SEP).append(step.getFlowId() != null ? flowAlias.get(step.getFlowId()) : "").append(SEP).append(step.getName()).append(SEP).append(step.getExecutionStatus())
			.append(SEP).append(DurationFormatUtils.formatDuration(step.getDuration(), "HH:mm:ss")).append(SEP).append(step.getStartTime() != null ? DateFormatUtils.format(step.getStartTime(), "yyyy-MM-dd HH:mm:ss") : "").append(SEP)
			.append(step.getEndTime() != null ? DateFormatUtils.format(step.getEndTime(), "yyyy-MM-dd HH:mm:ss") : "").append(SEP).append(EOL);
	    }
	}
