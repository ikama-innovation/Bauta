...
public class SchedulingView extends VerticalLayout implements SelectionListener<Grid<JobTrigger>, JobTrigger> {
	...
	Button removeButton, editButton, addCronButton, addJobCompletionButton, addJobCompletionOrFailureButton,
			importButton;
	...
	public SchedulingView() {
		...
		add(buttons);
		lAdminInfo = new Span("You need to be ADMIN in order to edit triggers");
	...
	private com.vaadin.flow.component.Component createTimestampColumn(Date timestamp) {
		...
		var label = new Span(tstampFormat.format(timestamp));
	...
	private void addJobCompletionOrFailure(JobTrigger jobTrigger) {
		Dialog jobCompletionDialog = createAddJobCompletionDialog(JobTrigger.TriggerType.JOB_COMPLETION_OR_FAILURE,
				jobTrigger);
		jobCompletionDialog.open();
	}
