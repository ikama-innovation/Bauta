...
public class SchedulingView extends VerticalLayout implements SelectionListener<Grid<JobTrigger>, JobTrigger> {
    ...
    public SchedulingView() {
        ...
        Grid.Column<JobTrigger> jobNameColumn = triggerGrid
                .addColumn(JobTrigger::getJobName).setHeader("Job Name").setSortable(true);
        ...
        Grid.Column<JobTrigger> triggeringJobColumn = triggerGrid
                .addColumn(JobTrigger::getTriggeringJobName).setHeader("Triggered by").setSortable(true);
        Grid.Column<JobTrigger> idColumn = triggerGrid
                .addColumn(JobTrigger::getId).setHeader("ID").setSortable(true);
        idColumn.setVisible(false);
        Grid.Column<JobTrigger> jobParamsColumn = triggerGrid
                .addColumn(JobTrigger::getJobParameters).setHeader("Job Parameters").setSortable(true);
        ...
        Grid.Column<JobTriggerLog> logTstampColumn = logGrid
                .addComponentColumn(item -> createTimestampColumn(item.getTstamp())).setHeader("Timestamp").setAutoWidth(true);
        Grid.Column<JobTriggerLog> logStatus = logGrid
                .addComponentColumn(item-> createStatusColumn(item.getStatus())).setHeader("Status").setAutoWidth(true);
        ...
        Grid.Column<JobTriggerLog> logErrorMsg = logGrid
                .addComponentColumn(item -> createLogColumn(item.getErrorMsg())).setHeader("Error").setAutoWidth(true);

        Grid.Column<JobTriggerLog> logTypeColumn = logGrid
                .addComponentColumn(item -> createTypeComponent(item.getTriggerType(), true)).setHeader("Type").setAutoWidth(true);
        ...
        Grid.Column<JobTriggerLog> logTriggeringJobColumn = logGrid
                .addComponentColumn(item-> createLogColumn(item.getTriggeringJobName())).setHeader("Triggered by").setAutoWidth(true);
        ...
        div.getStyle().set("font-family","monospace");
        add(div);
        */
    ...
    private String validateKeyValues(String s) {
        ...
        else if (!s.matches("([A-Za-z0-9_-]*=[A-Za-z0-9_\\-\\(\\):\\/\\\\\\*\\.]*)((,[ ]{0,1})([A-Za-z0-9_-]*=[A-Za-z0-9_\\-\\(\\):\\/\\\\\\*\\.]*))*")) {
